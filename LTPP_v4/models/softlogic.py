...
class SoftLogicEngine(nn.Module):
    ...
    def eval_single_clause_with_match(self, Theta_f, val_vec, T_match=None, tau=None):
        """
        改进的规则评估方法，更好地处理多个body谓词的conjunction
        """
        if T_match is None:
            T_match = self.T_match
        if tau is None:
        ...
        Kn = F.normalize(self.K, dim=1)  # (P, d)
        Thetan = F.normalize(Theta_f, dim=0)  # (d, h)
        S = Kn @ Thetan  # (P, h) - similarity scores

        # 改进的匹配策略
        h = S.shape[1]  # number of body slots
        ...
        for slot_idx in range(h):
            ...
            # 记录confidence（相似度）和truth value
            confidence = sim_scores[best_pred_pos]
            truth_value = val_vec[best_pred_pos]
        ...
        # 计算conjunction的满足度
        # 使用softmin来近似AND操作
        conf_tensor = torch.stack(slot_confidences)  # (h,)
        val_tensor = torch.stack(slot_values)  # (h,)

        # 组合confidence和truth values
        combined = torch.cat([conf_tensor, val_tensor], dim=0)  # (2h,)

        # 使用softmin计算最终的g值
        g_and = self.softmin_tau(combined, tau)
    ...
    def forward_chaining_with_trace(self, Thetas, initial_val, max_iters=6, tol=1e-6, return_W=False,
        ...
        if debug_verbose:
            ...
            print(f"Head to clauses mapping: {head_to_clauses}")
        ...
        for it in range(max_iters):
            ...
            for head, clause_keys in head_to_clauses.items():
                ...
                for key in clause_keys:
                    ...
                    g, _, argmax_preds = self.eval_single_clause_with_match(Theta_f, val)
                    clause_results.append((key, g, argmax_preds))

                    if debug_verbose:
                        print(f"  Evaluating clause {key}: g={g.item():.6f}, matched={argmax_preds}")

                # 如果有多个子句定义同一个head，使用soft-OR聚合
                if len(clause_results) == 1:
                    key_chosen, g_head, argmax_chosen = clause_results[0]
                else:
                    # 使用soft-OR (LogSumExp)
                    g_list = [result[1] for result in clause_results]
                    g_stack = torch.stack(g_list)

                    # 找到g值最高的子句作为主要贡献者
                    max_idx = int(torch.argmax(g_stack).item())
                    key_chosen, _, argmax_chosen = clause_results[max_idx]
                ...
                # 更新所有子句的g_cache
                for key, g, _ in clause_results:
                    g_cache[key] = float(g.item())

            val = val_new

            if debug_verbose and iteration_changes:
                print(f"Iteration {it} changes:")
                for head, old, new, delta in iteration_changes:
                    print(f"  {head}: {old:.6f} -> {new:.6f} (Δ={delta:.6f})")
